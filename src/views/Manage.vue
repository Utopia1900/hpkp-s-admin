<template>
  <div>
      ...
      <v-card>
        <v-card-text>
          <v-list>
            ...
            >{{clickedRoomInfo.bNo}}栋-{{clickedRoomInfo.uNo}}单元-{{clickedRoomInfo.rNo}}
            </v-list-tile>
    ...
    <v-dialog v-model="cancelDirectDialog" max-width="300px">
      <v-card style="padding-top: 30px;background: #d6d6d6fa;color: #000;">确定要撤销【一键销控】吗？
        <v-card-actions>
          <v-spacer></v-spacer>
    ...
    <v-dialog v-model="clearNotRigidDialog" max-width="300px">
      <v-card style="padding-top: 30px;background: #d6d6d6fa;color: #000;">确定要清除【非刚需房源】吗？
        <v-card-actions>
          <v-spacer></v-spacer>
    ...
    <v-dialog v-model="cancelNotRigidDialog" max-width="300px">
      <v-card style="padding-top: 30px;background: #d6d6d6fa;color: #000;">确定要撤销【非刚需房源】吗？
        <v-card-actions>
          <v-spacer></v-spacer>
    ...
    <v-dialog v-model="cancelBuyDialog" max-width="300px">
      <v-card style="padding-top: 30px;background: #d6d6d6fa;color: #000;">确定要撤销【销控】吗？
        <v-card-actions>
          <v-spacer></v-spacer>
    ...
    <v-dialog v-model="cancelAssignDialog" max-width="300px">
      <v-card style="padding-top: 30px;background: #d6d6d6fa;color: #000;">确定要撤销【关联】吗？
        <v-card-actions>
          <v-spacer></v-spacer>
    ...
    <v-dialog v-model="clearDirectDialog" max-width="300px">
      <v-card style="padding-top: 30px;background: #d6d6d6fa;color: #000;">确定要清除【一键销控】吗？
        <v-card-actions>
          <v-spacer></v-spacer>
    ...
    <v-dialog v-model="clearAssignDialog" max-width="300px">
      <v-card style="padding-top: 30px;background: #d6d6d6fa;color: #000;">确定要清除【关联客户】吗？
        <v-card-actions>
          <v-spacer></v-spacer>
    ...
    <v-dialog v-model="guestDialog" persistent scrollable max-width="700px">
      ...
      <v-card>
        ...
        <div style="display:flex; padding-left:20px;">
          ...
          <v-btn color="green darken-1" style="color: #fff;" @click.native="searchGuest('guest')">查询客户</v-btn>
        </div>
          ...
          <v-list v-for="(item, index) in guestList" style="padding: 0" :key="index">
            ...
            >
              ...
              <span style="width: 20%;display: inline-block"
                    :style="{color:item.disabled?'#E53935': '#512DA8'}">{{formatStatus(item.disabled)}}</span>
            </v-list-tile>
          </v-list>
    ...
    <v-dialog v-model="assignGuestDialog" persistent scrollable max-width="500px">
      ...
      <v-card>
        ...
        <div style="display:flex; padding-left:20px;">
          <v-text-field v-model="name" label="输入客户姓名" style="padding-right:8px;"></v-text-field>
          <v-text-field v-model="mobile" label="输入手机号" style></v-text-field>
          <v-btn color="green darken-1" style="color: #fff;" @click.native="searchGuest('assignGuest')">查询客户</v-btn>
        </div>
    ...
    <div id="coverContainer" style="position: fixed;top: 0px;width: 100%;height: 100%;background: #00000091"
         v-show="cover">
      <div class="loader" style="visibility: visible">
        <svg viewBox="0 0 32 32" width="50" height="50">
          <circle id="spinner" cx="16" cy="16" r="15" fill="none"></circle>
        </svg>
      </div>
    </div>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      goToQuerySold() {
        ...
        // window.open(`http://localhost:3333?token=${token}`);
      },
      formatMoney(s, n) {
